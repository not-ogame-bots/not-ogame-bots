  ...
  def getInitialActions(implicit clock: LocalClock): IO[List[ScheduledAction[IO]]] = {
    val listOfActions = List(
      ...
      new BuildBuildingsOgameAction[IO](
        planet5,
        List(new FacilityBuildingTask(RoboticsFactory, 10), new FacilityBuildingTask(NaniteFactory, 2))
      ),
      new BuildBuildingsOgameAction[IO](
        planet6,
        List(new FacilityBuildingTask(RoboticsFactory, 10), new FacilityBuildingTask(NaniteFactory, 1))
      ),
      new ExpeditionOgameAction[IO](
        moon,
        Map(Destroyer -> 1, EspionageProbe -> 1, LargeCargoShip -> 400, Explorer -> 20),
