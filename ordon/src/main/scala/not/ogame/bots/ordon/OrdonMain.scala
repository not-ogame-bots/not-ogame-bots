...
object OrdonMain extends IOApp {
  ...
  private def process(ogame: OgameDriver[IO], scheduled: IO[List[ScheduledAction[IO]]]): IO[Unit] = {
    ...
    val processed: IO[List[ScheduledAction[IO]]] = scheduledActionList.map(process(_, ogame, LocalDateTime.now())).sequence.map(_.flatten)
    val withSleep = processed
      .flatMap(a => IO.sleep(1 second).map(_ => a))
    process(ogame, withSleep)
  }

  private def process(scheduledAction: ScheduledAction[IO], ogame: OgameDriver[IO], now: LocalDateTime): IO[List[ScheduledAction[IO]]] = {
