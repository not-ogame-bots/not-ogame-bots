...
class StateAggregator(state: Ref[Task, State], notifier: Notifier)(implicit clock: LocalClock) {
  def run(): Task[Unit] = {
    notifier.subscribeToNotifications.consumeWith(Consumer.foreachTask {
