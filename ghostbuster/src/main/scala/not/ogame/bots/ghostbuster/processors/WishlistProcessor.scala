...
//import not.ogame.bots.ghostbuster.{BotConfig, PlanetState, RandomTimeJitter, State, Action, Wish}
...
//  }
//
//  private def process(planetState: PlanetState, tasks: List[Action]): List[Action] = {
...
//      List(Action.RefreshSupplyAndFacilityPage(planetState.suppliesPage.currentBuildingProgress.get.finishTimestamp, planetState.id))
//    } else {
//      List.empty
//    }
//  }
...
//          if suppliesPage.suppliesLevels.values(w.suppliesBuilding).value < w.level.value && w.planetId == planetState.id =>
...
//        Some(Action.RefreshFleetOnPlanetStatus(clock.instant(), planetState.id))
//      case w: Wish.BuildShip if planetState.fleetOnPlanet.contains(w.shipType) =>
...
//      Some(Action.BuildFacility(buildWish.facility, nextLevel(facilitiesBuildingLevels, buildWish.facility), clock.instant(), planetId))
//    } else if (botConfig.allowWaiting) {
//      if (suppliesPage.currentCapacity.gtEqTo(requiredResources)) {
//        val stillNeed = requiredResources.difference(suppliesPage.currentResources)
//        val hoursToWait = stillNeed.div(suppliesPage.currentProduction).max
//        val secondsToWait = (hoursToWait * 3600).toInt + jitterProvider.getJitterInSeconds()
//        val timeOfExecution = clock.instant().plusSeconds(secondsToWait)
//        Some(Action.BuildFacility(buildWish.facility, nextLevel(facilitiesBuildingLevels, buildWish.facility), timeOfExecution, planetId))
//      } else {
